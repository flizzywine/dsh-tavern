...
- **外部预设库与破限方案库**：导入和阅读 SillyTavern 预设，从中抽取需要运行的破限提示词与正则；
...
需要 SillyTavern 消息结构的人物卡和破限方案，可以在设置中手动开启兼容模式。开启后，侧栏最右侧会出现“兼容（实验性）”；该模式不运行 DSH 后台 Agent、状态结算和候选项，实际兼容效果可能因预设、模型和供应商而异。
