...
- **预设库（实验性）**：导入并选择 SillyTavern 预设，可在详情中调整提示词和正则开关，也可引用到卡片对话中交给 Agent 编辑；
...
需要 SillyTavern 消息结构的人物卡，可以在设置中手动开启兼容模式。开启后，侧栏最右侧会出现“兼容（实验性）”。用户可以自行导入并选择一份外部预设；兼容模式会按预设原有的提示词顺序、启用状态和正则设置构造请求。预设也可以引用到卡片对话中，由 Agent 协助编辑。兼容效果可能因预设、模型和供应商而异。
