...
如果用户选择了外部预设，普通游玩会把可识别的启用提示词按头部、中部和尾部投影到 DSH 前台请求，并应用受支持的预设正则。历史仍使用 DSH 的追加式 Agent 架构，后台 Agent 与卡片 Agent 不运行该预设。这种方式只是允许使用预设，并不完整复刻 SillyTavern；部分占位、宏、顺序、参数和第三方扩展可能不兼容，也不保证破限效果。
