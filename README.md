...
- **预设库（实验性）**：导入并选择 SillyTavern 预设。普通游玩会将可识别内容投影到前台正文请求，兼容模式会进一步按 SillyTavern 消息结构编译；也可在详情中调整提示词和正则，或引用到卡片对话中交给 Agent 编辑；
...
Agent 游玩直接导入人物卡即可开始，不要求用户配置预设。

DSH Tavern 已自带原生运行预设。除非坚持要使用预设，否则建议保持“不使用预设”；开启用户导入的外部预设可能与原生行为冲突并导致系统异常，请用户自行判断。如果要调整内容偏好，建议通过改卡将偏好写入人物卡，或使用 Guide 注入偏好。

普通游玩可以使用外部预设，但不能保证完全适配 SillyTavern，也不保证达到破限效果。

#### 兼容模式（实验性）

需要更接近 SillyTavern 消息结构的人物卡，可以进入“设置 → DSH Tavern”，开启“兼容模式（实验性）”；开启后左侧会显示“兼容（实验性）”入口。兼容模式会尽最大可能复刻 SillyTavern 的消息结构与预设行为，按预设原有的提示词顺序、启用状态和正则设置构造请求，对预设的适配度高于普通游玩。预设也可以引用到卡片对话中，由 Agent 协助编辑。

#### MVU 人物卡支持

支持运行使用 MagVarUpdate（MVU）的人物卡。变量更新由后台 Agent 独立结算，正文下方会展示本轮更新结果；人物卡状态栏则常驻右侧“酒馆状态”面板，不再挤占正文历史。

当前为初步支持，实际效果仍取决于人物卡自身的脚本和变量结构。
