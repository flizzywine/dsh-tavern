...
- **预设库（实验性）**：导入和阅读 SillyTavern 预设，由用户自行勾选需要启用的提示词与正则；
...
需要 SillyTavern 消息结构的人物卡，可以在设置中手动开启兼容模式。开启后，侧栏最右侧会出现“兼容（实验性）”。用户可以自行导入外部预设，并在预设库中勾选需要启用的条目；系统不会直接运行整份外部预设。兼容效果可能因预设、模型和供应商而异。
