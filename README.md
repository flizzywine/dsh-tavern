...
剧本使用普通的 `.txt` 或 `.md` 文件即可，不要求专用格式。可以放完整小说、分幕剧本或按时间顺序整理的故事大纲；内容越清楚地写明场景、事件、人物行动与前后顺序，Agent 越容易稳定推进。

1. 准备人物卡，以及一份 `.txt` / `.md` 剧本文件。想先体验公开案例，可以使用 [`avra-complete.json`](demo/cards/avra-complete.json) 和 [`the-missing-silver-bell-caravan.md`](demo/scripts/the-missing-silver-bell-caravan.md)。
2. 打开左侧 **卡片**，点击 **＋ 新建 / 编辑人物卡**。如果人物卡还没有导入，先点击 **导入人物卡**。
3. 在目标人物卡右侧点击 **绑定剧本**，选择准备好的剧本文件。绑定成功后，人物卡下方会显示剧本名称和分块数量；需要更换时点击 **替换剧本**。
4. 切回左侧 **游玩**，点击 **＋ 选择人物卡 · 新开游玩**，选择刚才的人物卡。绑定了剧本的卡会自动创建剧本会话，不需要再打开额外开关。
5. 在新会话中输入行动并发送。右侧出现 **剧本进度** 和 **剧本预览**，游玩历史显示“剧本 · 人物名”，就说明剧本模式已经启动。

**绑定剧本只影响之后新建的会话。** 已经存在的自由游玩会话仍会保持自由模式；绑定后请新开一次游玩。解绑同样只影响之后的新会话，已有剧本会话及其进度会保留。

游玩时不必照抄剧本中的行动。你可以自由输入，Agent 会把玩家行动接入当前剧情；点击 **生成候选项** 时，剧本模式只提供一个最贴近主线的推荐行动。回退本轮会同时恢复正文、人物姿势和剧本进度。
