...
`;
		...
		// Temporary alpha diagnostics: bounded batches, no chat writes or retries.
		function createMvuRefreshTrace(options) {
			let queue = [], timer = null, sending = false;
			const pageId = "page-" + Date.now() + "-" + Math.random().toString(36).slice(2);
			function schedule() {
				if (timer !== null || sending || !queue.length) return;
				timer = options.schedule(flush, 1000);
			}
			async function flush() {
				timer = null;
				if (sending || !queue.length) return;
				const batch = queue; queue = []; sending = true;
				try { await options.send(batch); }
				catch (_) { try { options.fallback(batch); } catch (_) {} }
				finally { sending = false; schedule(); }
			}
			return function (stage, fields) {
				try {
					queue.push(Object.assign({}, fields || {}, { stage: stage, pageId: pageId, at: Date.now() }));
					if (queue.length > 100) queue.shift();
					schedule();
				} catch (_) {}
			};
		}
		const traceMvuRefresh = createMvuRefreshTrace({
			schedule: function (run, delay) { return window.setTimeout(run, delay); },
			send: function (entries) {
				const controller = new AbortController();
				const timer = window.setTimeout(function () { controller.abort(); }, 3000);
				return rpc("recordMvuRefreshDiagnostics", { entries: entries }, null, { signal: controller.signal }).finally(function () { window.clearTimeout(timer); });
			},
			fallback: function (entries) { console.info("[DEBUG-mvu-refresh-v1] delivery-failed", JSON.stringify(entries)); }
		});
		...
		function createLiveTavernViewModule(options) {
			...
			async function refresh(record) {
				...
				try {
					...
					if (view && view.tavernHelper) traceMvuRefresh("client-view", { sessionId: record.id, revision: Number(view.tavernHelper.stateRevision) || 0, messageCount: (view.tavernHelper.messages || []).length });
			...
			connect: function (sessionId, handlers) {
				...
				source.onmessage = function (event) {
					try {
						...
						traceMvuRefresh("client-sse", { sessionId: sessionId, revision: Number(sync.diagnosticRevision) || 0 });
						handlers.message(coordinationView({ sync: sync }, sessionId));
					}
					catch (error) { handlers.error(error); }
				};
				source.onerror = function () { traceMvuRefresh("client-sse-error", { sessionId: sessionId }); handlers.error(new Error("Tavern SSE 正在重连")); };
		...
		function tavernFrameRefreshProbe(token, readState, enabled) {
			if (!enabled) return function () {};
			function report(stage, update, event, count) {
				try {
					parent.postMessage({ type: "dsh-tavern-refresh-diagnostic", token: token, entry: {
						stage: stage, token: token, currentRevision: Number(readState().stateRevision) || 0,
						revision: Number(update && update.stateRevision) || 0, baseRevision: Number(update && update.baseRevision) || 0,
						event: event || "none", listenerCount: stage === "frame-dom" ? 0 : Number(count) || 0,
						mutationCount: stage === "frame-dom" ? Number(count) || 0 : 0
					} }, "*");
				} catch (_) {}
			}
			if (typeof MutationObserver === "function") {
				const observer = new MutationObserver(function (records) { report("frame-dom", null, null, records.length); });
		...
		function TavernMessageFrame(props) {
			...
			const desiredDocument = React.useMemo(function () {
				...
				const document = {
					...
					token: token,
					html: buildTavernFrameDocument({ content: props.content, token: token, helperContext: effectiveHelperContext, styleEnvironment: props.styleEnvironment, turn: props.turn, observeMvuView: props.observeMvuView, runtimeReporting: props.runtimeReporting, refreshDiagnostics: props.persistent === true }),
			...
			function sendLatestContext(frame, document, forceSnapshot) {
				...
				if (props.persistent) traceMvuRefresh(update ? "frame-send" : "frame-noop", { sessionId: props.sessionId, token: document.token, currentRevision: Number(current.context && current.context.stateRevision) || 0, revision: Number(latestHelperContextRef.current.stateRevision) || 0, baseRevision: Number(update && update.baseRevision) || 0, kind: update && update.kind || "none", turn: Number(latestTurnRef.current) || 0, operationCount: (update && update.operations || []).length, eventCount: (update && update.events || []).length });
				if (!update) return;
			...
			React.useEffect(function () {
				...
				if (props.persistent) traceMvuRefresh("frame-effect", { sessionId: props.sessionId, token: visible && visible.token, revision: Number(effectiveHelperContext && effectiveHelperContext.stateRevision) || 0, turn: Number(props.turn) || 0 });
			...
			React.useEffect(function () {
				function receive(event) {
					...
					if (props.persistent && sourceDocument && data.type === "dsh-tavern-refresh-diagnostic") {
						const entry = data.entry || {};
						// Only fixed metadata crosses this diagnostic boundary, never arbitrary frame text.
						if (["frame-receive", "frame-reject", "frame-applied", "frame-event-start", "frame-event-end", "frame-event-error", "frame-dom"].includes(entry.stage)) traceMvuRefresh(entry.stage, {
							sessionId: props.sessionId, token: data.token, revision: Number(entry.revision) || 0, baseRevision: Number(entry.baseRevision) || 0, currentRevision: Number(entry.currentRevision) || 0,
							event: ["MESSAGE_UPDATED", "MESSAGE_RECEIVED", "mag_variable_update_ended"].includes(entry.event) ? entry.event : "other", listenerCount: Number(entry.listenerCount) || 0, mutationCount: Number(entry.mutationCount) || 0
						});
						return;
					}
					if (props.persistent && (data.type === "dsh-tavern-frame-ready" || data.type === "dsh-tavern-helper-context-request")) traceMvuRefresh(data.type === "dsh-tavern-frame-ready" ? "frame-ready" : "frame-resync", { sessionId: props.sessionId, token: data.token });
		...
		exports.createMvuRefreshTrace = createMvuRefreshTrace;
