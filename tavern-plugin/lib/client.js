...
`;
		...
		function buildTavernFrameDocument(input) {
			...
			const touchBridge = '<script data-dsh-tavern-touch-bridge>(function(){var token=' + token + ',touch=null,axis="",pending=0,queued=false;function coordinate(point,axis){var screen=Number(point["screen"+axis]);return Number.isFinite(screen)?screen:(Number(point["client"+axis])||0);}function flush(){queued=false;var delta=pending;pending=0;if(delta)parent.postMessage({type:"dsh-tavern-frame-pan",token:token,deltaY:delta},"*");}function queue(delta){pending+=delta;if(queued)return;queued=true;if(typeof requestAnimationFrame==="function")requestAnimationFrame(flush);else flush();}function reset(){if(pending)flush();touch=null;axis="";}function scrollable(target,delta){var node=target;while(node&&node!==document.body&&node!==document.documentElement){var style;try{style=getComputedStyle(node);}catch(e){style=null;}var overflow=String(style&&(style.overflowY||style.overflow)||"visible");if((overflow==="auto"||overflow==="scroll"||overflow==="overlay")&&Number(node.scrollHeight)>Number(node.clientHeight)+1){if(delta>0&&Number(node.scrollTop)+Number(node.clientHeight)<Number(node.scrollHeight)-1)return true;if(delta<0&&Number(node.scrollTop)>1)return true;}node=node.parentElement;}var root=document.scrollingElement||document.documentElement;if(root&&Number(root.scrollHeight)>Number(root.clientHeight)+1){if(delta>0&&Number(root.scrollTop)+Number(root.clientHeight)<Number(root.scrollHeight)-1)return true;if(delta<0&&Number(root.scrollTop)>1)return true;}return false;}document.addEventListener("touchstart",function(event){if(!event.touches||event.touches.length!==1){reset();return;}var point=event.touches[0],x=coordinate(point,"X"),y=coordinate(point,"Y");touch={x:x,y:y,lastY:y};axis="";},{capture:true,passive:true});document.addEventListener("touchmove",function(event){if(!touch||!event.touches||event.touches.length!==1)return;var point=event.touches[0],x=coordinate(point,"X"),y=coordinate(point,"Y"),dx=x-touch.x,dy=y-touch.y;if(!axis){if(Math.max(Math.abs(dx),Math.abs(dy))<8)return;axis=Math.abs(dy)>Math.abs(dx)?"vertical":"horizontal";}if(axis!=="vertical")return;var delta=touch.lastY-y;touch.lastY=y;if(!delta||scrollable(event.target,delta))return;event.preventDefault();queue(delta);},{capture:true,passive:false});document.addEventListener("touchend",reset,{capture:true,passive:true});document.addEventListener("touchcancel",reset,{capture:true,passive:true});})();<\/script>';
			...
			return '<!doctype html><html><head><meta charset="utf-8">'
				...
				+ '</head><body class="no-blur">' + html + layoutNormalizer + touchBridge + reporter + readyReporter + '</body></html>';
		}
		...
		function scrollTavernFrameHost(frame, value, hostWindow) {
			const raw = Number(value);
			if (!frame || !Number.isFinite(raw) || raw === 0) return false;
			const delta = Math.max(-160, Math.min(160, raw));
			let node = frame.parentElement;
			while (node) {
				let style = null;
				try { style = hostWindow && typeof hostWindow.getComputedStyle === "function" ? hostWindow.getComputedStyle(node) : null; } catch (_) {}
				const overflow = String(style && (style.overflowY || style.overflow) || "visible");
				if ((overflow === "auto" || overflow === "scroll" || overflow === "overlay") && Number(node.scrollHeight) > Number(node.clientHeight) + 1) {
					node.scrollTop = Number(node.scrollTop || 0) + delta;
					return true;
				}
		...
		function TavernMessageFrame(props) {
			...
			React.useEffect(function () {
				function receive(event) {
					...
					if (data.type === "dsh-tavern-frame-pan" && sourceDocument && visible && data.token === visible.token) {
						scrollTavernFrameHost(frame, data.deltaY, window);
		...
		exports.scrollTavernFrameHost = scrollTavernFrameHost;
