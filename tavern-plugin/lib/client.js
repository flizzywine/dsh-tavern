...
`;
		...
		function TavernMessageFrame(props) {
			...
			function sendLatestContext(frame, token) {
				if (!frame || !frame.contentWindow || !latestHelperContextRef.current) return;
				frame.contentWindow.postMessage({ type: "dsh-tavern-helper-context", token: token, context: latestHelperContextRef.current, turn: latestTurnRef.current }, "*");
			}
			...
			React.useEffect(function () {
				...
				sendLatestContext(frame, visible.token);
			...
			React.useEffect(function () {
				function receive(event) {
					...
					if (data.type === "dsh-tavern-frame-ready") sendLatestContext(frame, data.token);
