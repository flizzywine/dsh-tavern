...
`;
		...
		function buildTavernFrameDocument(input) {
			...
			const touchBridge = '<script data-dsh-tavern-touch-bridge>(function(){var token=' + token + ',touch=null,axis="";function reset(){touch=null;axis="";}function scrollable(target,delta){var node=target;while(node&&node!==document.body&&node!==document.documentElement){var style;try{style=getComputedStyle(node);}catch(e){style=null;}var overflow=String(style&&(style.overflowY||style.overflow)||"visible");if((overflow==="auto"||overflow==="scroll"||overflow==="overlay")&&Number(node.scrollHeight)>Number(node.clientHeight)+1){if(delta>0&&Number(node.scrollTop)+Number(node.clientHeight)<Number(node.scrollHeight)-1)return true;if(delta<0&&Number(node.scrollTop)>1)return true;}node=node.parentElement;}var root=document.scrollingElement||document.documentElement;if(root&&Number(root.scrollHeight)>Number(root.clientHeight)+1){if(delta>0&&Number(root.scrollTop)+Number(root.clientHeight)<Number(root.scrollHeight)-1)return true;if(delta<0&&Number(root.scrollTop)>1)return true;}return false;}document.addEventListener("touchstart",function(event){if(!event.touches||event.touches.length!==1){reset();return;}var point=event.touches[0];touch={x:Number(point.clientX)||0,y:Number(point.clientY)||0,lastY:Number(point.clientY)||0};axis="";},{capture:true,passive:true});document.addEventListener("touchmove",function(event){if(!touch||!event.touches||event.touches.length!==1)return;var point=event.touches[0],x=Number(point.clientX)||0,y=Number(point.clientY)||0,dx=x-touch.x,dy=y-touch.y;if(!axis){if(Math.max(Math.abs(dx),Math.abs(dy))<8)return;axis=Math.abs(dy)>Math.abs(dx)?"vertical":"horizontal";}if(axis!=="vertical")return;var delta=touch.lastY-y;touch.lastY=y;if(!delta||scrollable(event.target,delta))return;event.preventDefault();parent.postMessage({type:"dsh-tavern-frame-pan",token:token,deltaY:delta},"*");},{capture:true,passive:false});document.addEventListener("touchend",reset,{capture:true,passive:true});document.addEventListener("touchcancel",reset,{capture:true,passive:true});})();<\/script>';
		...
		function createTavernMessageFrameLifecycle(initial, options) {
			...
			function receive(event) {
				...
				} else if (data.type === "dsh-tavern-frame-pan") {
					if (sourceDocument === visible) scrollTavernFrameHost(channel.element(), data.deltaY, hostWindow);
		...
		function CandidateDockActions(props) {
			...
			return h("div", { className: "dsh-tavern-dock-actions" },
				...
				isPlayMode(sessionMode) ? React.createElement(SceneImageAction, { key: props.sessionId + ":" + imageTurn, sessionId: props.sessionId, turn: imageTurn, running: running }) : null,
