			...
			function ExternalPresetLibraryTab(props) {
				...
				if (preset && preset.path === detailPath) return h("div", { className: "dsh-tavern-presets" },
					...
					h("div", { className: "dsh-tavern-preset-detail" }, error ? h("div", { className: "dsh-tavern-dock-error" }, error) : null,
						h("div", { className: "dsh-tavern-preset-summary" }, h("b", null, "编辑预设提示词和正则"), h("p", null, "点击条目展开编辑，保存后会直接写回预设文件。普通游玩和兼容模式会在下一轮请求中使用修改后的内容。"), h("p", null, "普通游玩只会把可识别的提示词按头部、中部和尾部投影到 DSH 前台请求，并应用受支持的正则；这不是完整的 SillyTavern 运行环境。"), h("p", null, "卡片模式中的引用只供 Agent 阅读和编辑，后台 Agent 也不会运行预设。")),
