...
- Executable conformance slice: `tests/tavern-mvu-upstream-conformance.test.mjs` freezes the successful add/replace/remove JSON Patch vectors and parser recovery cases registered by the pinned upstream tests. This proves only those named behaviors; it does not upgrade the integration to full MagVarUpdate compatibility.
