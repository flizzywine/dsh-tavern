...
- Executable conformance slice: `tests/tavern-mvu-upstream-conformance.test.mjs` freezes the successful add/replace/remove JSON Patch vectors, parser recovery cases, the missing-leading-slash path case, and lodash command validation/metadata cases registered by the pinned tests. This proves only those named behaviors; it does not upgrade the integration to full MagVarUpdate compatibility.
