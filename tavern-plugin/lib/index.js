...
export function apply(ctx) {
  ...
  function buildSystem(card, chat, scriptReference, worldBookIds) {
    ...
    if (scriptReference !== null && scriptReference !== undefined && str(scriptReference.text) !== '') {
      ...
      parts.push('【剧本模式要求】\n剧本仅供参考，不必逐句照搬或插入原文台词；剧本顺序也不必严格照搬，只要逻辑合理，允许按当前剧情调整先后；自然承接玩家行动并引向剧本方向，参考剧本遣词造句但允许自由发挥；剧情尽可能自然遵循剧本，玩家偏离时可自然偏离，再找机会回来；同一句话不要出现两遍；如果原文动作或台词与当前已发生情节冲突，以当前情节的连贯合理为先，可省略或改写；不要解释你在参考剧本。\n【连贯性要求】开头先承接上一段正文的结尾，再进入本轮指令和剧本事件；不跳事件，不重复已发生动作，不出现前后位置或动作矛盾。')
    }
    return parts.join('\n\n')
  }
