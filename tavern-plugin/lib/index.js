...
export function apply(ctx) {
  ...
  function buildSystem(card, chat, scriptReference, worldBookIds) {
    ...
    if (scriptReference !== null && scriptReference !== undefined && str(scriptReference.text) !== '') {
      ...
      parts.push('【剧本模式要求】\n真正要顺着写的是用户指令，剧本仅供参考：可以参考剧本的剧情和遣词造句，但不强求贴合，可以跳过或调整剧本顺序。不要为了贴剧本而偏离、替换或补写指令之外的主干剧情；允许自由发挥；同一句话不要出现两遍，同一动作/台词只演一次；如果原文与当前情节冲突，以当前情节和用户指令的连贯合理为先，可省略或改写；不要解释你在参考剧本。\n【连贯性要求】开头先承接上一段正文的结尾，把用户指令和剧本参考融合成一段连续叙述，不要分成“先演指令、再演剧本”的两段；不跳事件，不重复已发生动作，不出现前后位置或动作矛盾。')
    }
    return parts.join('\n\n')
  }
