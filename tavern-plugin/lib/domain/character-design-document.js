...
export const CHARACTER_DESIGN_READ_TOOL = Object.freeze({
  ...
  description: '读取当前对话的人物设计档案。无参数时只返回精简索引；需要复用或修改某人时按 characterId 或姓名读取完整方案。',
...
export const CHARACTER_DESIGN_SAVE_TOOL = Object.freeze({
  ...
  description: '保存当前对话的重要人物完整人物方案；先读取并复用现有档案，完整设计完成后调用，再把方案映射到 MVU 变量。',
  parameters: Object.freeze({
    type: 'object',
    additionalProperties: false,
    properties: {
  ...
  function read(args) {
    ...
    if (requested) {
      ...
      return character === undefined
        ...
        : { ok: true, found: true, character: clone(character) }
    }
    return {
      ok: true,
