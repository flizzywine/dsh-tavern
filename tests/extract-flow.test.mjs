...
test('Tavern 对话顶栏显示当前启用的预设条目来源，未启用时显示无', () => {
  const player = between(clientSource, 'function TavernPlayerNameAction', 'function TavernStatusPanel')
  ...
  assert.match(view, /bypassPlan: activeBypassSnapshot === null \? null : \{ id: activeBypassSnapshot\.planId, name: activeBypassSnapshot\.presetName \|\| activeBypassSnapshot\.planName \}/)
  assert.match(player, /const planName = view\.bypassPlan && view\.bypassPlan\.name \? view\.bypassPlan\.name : "无"/)
  assert.match(player, /"预设条目：" \+ planName/)
...
test('预设库直接选择并激活外部预设条目，不暴露独立方案产品', () => {
  ...
  assert.match(panel, /只会使用你手动启用的条目，不会直接运行整份预设/)
  assert.match(panel, /当前仅兼容模式完成外部预设条目适配；游玩模式和卡片模式尚未完成适配，请勿使用/)
  assert.match(panel, /开启兼容模式：进入设置，在“DSH Tavern”中开启“启用兼容模式（实验性）”/)
  assert.match(panel, /启用的条目可能干扰 DSH Tavern 的正常运行，请谨慎选择/)
  assert.match(panel, /dsh-tavern-external-preset-notice/)
  assert.match(panel, /dsh-tavern-preset-warning/)
  assert.match(panel, /选择条目/)
  assert.match(panel, /启用所选条目/)
  assert.match(panel, /停用预设条目/)
  assert.match(panel, /existingPlanForPreset/)
  assert.match(panel, /setEntryKeys/)
  assert.match(panel, /setRegexKeys/)
  assert.match(panel, /提示词默认不选；正则默认全部迁移并继承原启用状态/)
  assert.match(panel, /rpc\("extractBypassPlan"/)
  assert.match(panel, /rpc\("activateBypassPlan"/)
  assert.match(panel, /entryKeys: entryKeys, regexKeys: regexKeys/)
  assert.match(panel, /dsh-tavern-extract-state/)
  assert.match(panel, /checked \? "启用" : "不启用"/)
...
test('设置开启后在顶层侧栏最右侧显示实验性兼容模式', () => {
	...
	assert.match(shell, /支持使用预设库中手动启用的条目，但不直接运行整份预设/)
	assert.match(shell, /需要哪个条目，在预设库中勾选并启用即可/)
