...
test('预设库选择整份预设，并可交给卡片 Agent 编辑或在当前对话引用', () => {
  ...
  assert.match(panel, /预设原有顺序使用其中已启用的提示词和正则/)
  assert.match(panel, /修改预设后，下一轮请求直接生效/)
  assert.match(panel, /在卡片模式中引用预设只供 Agent 阅读和编辑，不会自动选择或运行该预设/)
  ...
  assert.match(panel, /"停用预设"/)
  assert.match(panel, /"在对话中引用"/)
  assert.match(panel, /props\.appendMention\("preset"/)
  assert.match(panel, /"← 返回预设库"/)
  assert.match(panel, /编辑预设自身的启用状态/)
  assert.match(panel, /修改会直接保存到预设文件。当前预设仅在兼容模式中生效，下一轮兼容请求会使用新的提示词与正则状态。/)
