...
test('预设库独立导入、阅读并控制全局运行时条目', () => {
  ...
  assert.match(panel, /已勾选|未勾选/)
  assert.match(panel, /rpc\("togglePresetEntry"/)
  assert.match(panel, /rpc\("togglePresetRegex"/)
  assert.match(panel, /rpc\("selectPreset"/)
  assert.match(panel, /rpc\("disableAllPresets"/)
  assert.match(panel, /rpc\("savePresetPlan"/)
  assert.match(panel, /rpc\("applyPresetPlan"/)
  assert.match(panel, /rpc\("renamePresetPlan"/)
  assert.match(panel, /rpc\("deletePresetPlan"/)
  assert.match(panel, /预设配置方案/)
  assert.match(panel, /保存当前配置/)
  assert.match(panel, /所有外部预设内容导入后默认关闭/)
  assert.match(panel, /外部预设内容可能破坏系统正常行为，请谨慎开启/)
  assert.match(panel, /预设的提示词修改需新建对话后生效/)
  assert.match(panel, /预设的正则修改会立即生效/)
  assert.match(panel, /正则脚本/)
  assert.match(panel, /当前启用预设/)
  assert.match(panel, /不启用外部预设/)
  assert.match(panel, /所有内容导入后默认关闭，可按需勾选/)
  assert.match(panel, /已勾选.*条/)
