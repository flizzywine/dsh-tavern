...
test('预设库选择整份预设，并可交给卡片 Agent 编辑或在当前对话引用', () => {
  ...
  assert.match(panel, /建议保留内置预设/)
  assert.match(panel, /优先在卡片模式中写入人物卡，或在游玩中通过 Guide 注入要求/)
  assert.match(panel, /外部预设会参与前台请求并可能改变系统行为/)
  ...
  assert.match(panel, /三段代表提示词进入前台请求的真实位置/)
  assert.match(panel, /保存后下一轮游玩直接生效/)
  ...
  assert.match(panel, /位于系统上下文和历史之前/)
  assert.match(panel, /位于历史之后、本轮输入之前/)
  assert.match(panel, /并入本轮输入末尾/)
