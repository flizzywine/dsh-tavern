...
test('开场白创建失败时在选择弹窗内持续显示具体阶段和错误', () => {
  ...
  assert.match(playFlow, /let phase = "清理当前空白对话"/)
  assert.match(playFlow, /phase = "创建 DSH Session"/)
  assert.match(playFlow, /phase = "写入人物卡开场白"/)
  assert.match(playFlow, /setError\(phase \+ "失败："/)
})
...
test('新建对话把缺失的当前空 Session 视为已归档', () => {
  ...
  assert.match(playFlow, /await archiveCurrentBlankSession\(\)/)
  assert.match(cardFlow, /await archiveCurrentBlankSession\(\)/)
})
