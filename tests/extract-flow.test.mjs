...
test('外部预设只读，抽取时提示词手选且正则默认全部迁移', () => {
  ...
  assert.match(panel, /h\("strong", null, "抽取为破限方案后使用"\)/)
  assert.match(panel, /如需控制文风、字数、人称、叙事节奏等内容偏好/)
  assert.match(panel, /h\("strong", null, "卡片模式"\)/)
  assert.match(panel, /h\("strong", null, "系统提示"\)/)
  assert.match(panel, /h\("strong", null, "历史后指令"\)/)
  assert.match(panel, /openExtraction\(item\)/)
  assert.match(panel, /setPreset\(source\); startExtraction\(source\)/)
  assert.match(panel, /setEntryKeys\(\[\]\)/)
  assert.match(panel, /setRegexKeys\(\(source\.extractableRegexScripts \|\| \[\]\).*script\.regexKey/s)
  ...
  assert.match(panel, /适配模型（选填，多个模型用逗号分隔）/)
  assert.match(panel, /compatibleModelsFromText\(compatibleModelsText\)/)
  assert.match(panel, /setBypassPlanCompatibleModels/)
  assert.match(panel, /适配模型未标注/)
  assert.match(panel, /请仅在兼容模式中使用破限方案/)
  assert.match(panel, /尚未适配游玩模式和卡片模式/)
