...
test('后台结算期间禁用候选项按钮，完成后自动恢复', () => {
  ...
  assert.match(action, /tavernActivity\.setView\(props\.sessionId, \{ phase: "running", busy: true, role: "candidate"/)
  assert.doesNotMatch(action, /generateChoices[^\n]+(?:signal|timeout|AbortController)/)
  assert.match(action, /finally \{ tavernActivity\.invalidate\(props\.sessionId\); liveTavernView\.invalidate\(props\.sessionId\); setBusy\(false\); \}/)
