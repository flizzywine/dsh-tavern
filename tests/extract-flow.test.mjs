...
test('左侧栏常驻显示版本与构建号，并把检查更新和进行更新分成两步', () => {
  ...
  assert.match(sidebar, /jsDelivr 备用源未发现可用的新构建/)
...
test('人物卡 iframe 不因等价上下文或切换会话而重复启动', () => {
	...
	assert.doesNotMatch(frame, /key: JSON\.stringify\(\[props\.content, helperContextKey/)
	assert.match(frame, /dsh-tavern-helper-context/)
	assert.match(frame, /result && result\.captured === true/)
	assert.match(frame, /frozenHelperContext/)
	assert.match(frame, /pendingDocument/)
	assert.match(frame, /dsh-tavern-frame-ready/)
	assert.match(frame, /opacity: 0/)
	assert.match(frame, /IntersectionObserver/)
	assert.match(frame, /sessionStorage\.setItem\(pending\.heightKey/)
	assert.match(frame, /activated \? \[renderFrame\(visibleDocument, false\), renderFrame\(pendingDocument, true\)\]/)
...
test('设置开启后在顶层侧栏最右侧显示实验性兼容模式', () => {
	...
	assert.match(clientSource, /pollTavernHelperEvent", \{ runtimeId: runtimeId, ready: runtimeReady \}/)
...
test('隔离 Helper iframe 可以只读加载已锁定的本机远程资源', () => {
	...
	assert.match(handler, /if \(!readsCachedAsset && !readsStaticAsset && !readsOfficialMvu && typeof origin === 'string'/)
