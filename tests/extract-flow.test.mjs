...
test('DSH Session 失联时保留输入并重载恢复，但绝不自动发送', () => {
  ...
  assert.match(timeout, /probe: function \(sessionId\) \{ return rpc\("getSessionConnection", \{\}, sessionId\); \}/)
  assert.match(timeout, /props\.connection\.api\.sessions\.models\(\{ sessionId: sessionId \}\)/)
  assert.match(timeout, /composerGate\.set\(props\.conversation, props\.sessionId, "connection"/)
...
test('后台结算期间禁用候选项按钮，完成后自动恢复', () => {
  ...
  const coordinator = between(clientSource, 'function createCandidateGenerationCoordinator', 'function createPlayControlsFeatureModule')
  const generationAdapter = between(clientSource, 'const candidateGenerationCoordinator', 'async function generateCandidateChoices')
  const generation = between(clientSource, 'async function generateCandidateChoices', 'const regenPanel')
  ...
  assert.match(clientSource, /function TavernActivityGate/)
  assert.match(action, /useTavernActivity\(props\.sessionId/)
  assert.match(action, /disabled:.*activity\.busy/s)
  assert.match(clientSource, /"后台结算中…"/)
  assert.match(action, /props\.refreshSessions\(\)/)
  assert.match(action, /generateCandidateChoices\(props\.sessionId, props\.messageId, guidance\)/)
  assert.match(guide, /generateCandidateChoices\(props\.sessionId, messageId, guide\)/)
  assert.doesNotMatch(clientSource, /rpc\("generateChoices"/)
  assert.match(generationAdapter, /rpc\("startChoices"/)
  ...
  assert.match(coordinator, /requestId: id\(\)/)
  assert.match(coordinator, /started = await query\(function \(request\) \{ return options\.start\(startInput, request\)/)
  assert.match(coordinator, /operation\.terminal === true/)
  assert.match(coordinator, /operation\.successful === true/)
  assert.match(coordinator, /Promise\.race\(\[observeResult\(\), observeOperation\(\)\]\)/)
  assert.match(coordinator, /candidates\.requestId/)
  assert.match(coordinator, /return options\.read\(startInput, request\)/)
  assert.match(generation, /liveTavernView\.invalidate\(sessionId\)/)
  assert.match(generationAdapter, /queryTimeoutMs: 2000/)
  assert.doesNotMatch(coordinator, /Tavern 状态同步超时/i)
