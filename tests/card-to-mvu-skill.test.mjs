...
test('人物设计是后台专用的独立内置 Skill', async () => {
  ...
  assert.match(skill.content, /当前变量、变量模板与 Schema[^\n]*\[mvu_update\][^\n]*现有人物/)
  assert.match(skill.content, /人物相关字段区分为设计字段、状态字段、派生字段或只读字段/)
  assert.match(skill.content, /符合当前模板的键名、层级、类型和校验约束/)
  assert.match(skill.content, /不预设字段名、层级、容器类型或更新路径/)
