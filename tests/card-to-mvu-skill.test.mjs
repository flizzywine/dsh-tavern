...
test('人物设计是后台专用的独立内置 Skill', async () => {
  ...
  assert.match(skill.content, /设计前读取本轮提供的【变量结构】/)
  assert.match(skill.content, /`mvuPath`/)
  assert.match(skill.content, /`mvuFields`/)
  assert.match(skill.content, /MVU 模板提到的字段是人物设计的最低覆盖范围/)
