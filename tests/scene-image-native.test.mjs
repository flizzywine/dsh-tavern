...
test('原生 DSH 仅接收已就绪可视变量，引用衣着片段并跨重启复用，不写前台', { skip: !process.env.DSH_BOOT_MODULE }, async t => {
  ...
  await finished()
  assert.equal(runtime.requests.length, 4, 'repaint does not rerun the text model or pick up future clothes')
...
test('原生 DSH 生图 Agent 按需读历史设定、引用片段，前台不注入且重画不重读', { skip: !process.env.DSH_BOOT_MODULE }, async t => {
  ...
  const first = await finish()
  assert.equal(runtime.requests.length, 5, 'lookup, character, layout, confirmation and acknowledgement stay in one native Agent task')
...
test('真实 DSH 子 Agent 调用生图工具，HTTP 返回图经宿主校验落盘，前台不新增消息', { skip: !process.env.DSH_BOOT_MODULE }, async t => {
  ...
  assert.equal(runtime.requests.length, 3, 'repaint must not call the text model')
  assert.equal(runtime.imageRequests.length, 2)
  ...
  assert.deepEqual(runtime.requests[3].tools.map(tool => tool.name), ['character_design_read', 'submit_image_adjustment'])
  assert.equal(adjusted.traceSessionId, status.traceSessionId, 'adjustment resumes the original child after runner disposal')
  assert.match(JSON.stringify(runtime.requests[3].messages), /左手轻轻搭着窗框/, 'original planning history remains in the child')
  ...
  assert.equal(runtime.requests.length, 5, 'global style is composed without another Agent request')
