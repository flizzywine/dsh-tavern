...
function confirmationReceipt(runtime, sessionId) {
  const results = runtime.traceEvents(sessionId).filter(event => event.type === 'tool/result')
    .flatMap(event => event.data.message.content)
    .filter(block => block.type === 'tool-result' && block.toolCallId === 'image-call')
  assert.ok(results.length > 0, '确认工具回执必须保存在真实 DSH 子会话中')
  return JSON.parse(results.at(-1).content.filter(block => block.type === 'text').map(block => block.text).join(''))
}
...
test('原生 DSH 仅接收已就绪可视变量，引用衣着片段并跨重启复用，不写前台', { skip: !process.env.DSH_BOOT_MODULE }, async t => {
  ...
  await finished()
  assert.equal(runtime.requests.length, 3, 'repaint does not rerun the text model or pick up future clothes')
...
test('原生 DSH 生图 Agent 按需读历史设定、引用片段，前台不注入且重画不重读', { skip: !process.env.DSH_BOOT_MODULE }, async t => {
  ...
  const first = await finish()
  assert.equal(runtime.requests.length, 4, 'lookup, character, layout and confirmation finish without an extra acknowledgement request')
...
test('真实 DSH 确认工具收到生图服务拒绝详情，不谎报图片完成也不自动重发', { skip: !process.env.DSH_BOOT_MODULE }, async t => {
  ...
  assert.match(JSON.stringify(receipt), /Unsupported size/)
  assert.match(JSON.stringify(receipt), /未自动重试/)
  assert.equal(runtime.requests.length, 2, '服务拒绝写入真实子会话，不增加确认请求')
  assert.equal(runtime.imageRequests.length, 1)
...
test('真实 DSH 子 Agent 调用生图工具，HTTP 返回图经宿主校验落盘，前台不新增消息', { skip: !process.env.DSH_BOOT_MODULE }, async t => {
  ...
  assert.equal(receipt.versionId, status.versions[0].id)
  assert.match(receipt.message, /图片已生成并保存/)
  ...
  assert.equal(runtime.requests.length, 3, 'global style is composed without another Agent request')
