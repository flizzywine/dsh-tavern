...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/') {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' }).end(`<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>场景生图完整流程验证</title><style>:root{--dsw-alias-label-primary:#222;--dsw-alias-label-secondary:#666;--dsw-alias-border-l2:#ddd;--dsw-specific-sidebar-fill:#fff}body{font:16px sans-serif;max-width:880px;margin:32px auto;padding:12px}${css}</style><h1>场景生图验证 · 测试图片</h1><div id="app"></div><hr><div id="settings"></div><hr><button id="restart">模拟重启并刷新</button> <button id="swipe">切换正文版本</button> <button id="fail">下一张模拟失败</button> <button id="evidence">核对调用记录</button><pre id="result"></pre><script src="/runner.js"></script>`)
      return
    }
