...
const characterDesignTasksSource = await readFile(new URL('../tavern-plugin/lib/domain/character-design-tasks.js', import.meta.url), 'utf8')
...
test('卡片 Agent 以极简模式工具为底座，游玩 Agent 不暴露文件或 Skill 工具', () => {
  ...
  assert.doesNotMatch(orchestratorSource, /if \(mode === 'script'\) return \[[^\]]*'bash'/)
	assert.match(serverSource, /controlledToolNames = new Set\(\['bash', 'pwsh', 'str_replace_editor', 'skill', 'web_search', 'tavern_save_skill', \.\.\.cordisToolNames, CHARACTER_DESIGN_REQUEST_TOOL\.name, 'tavern_user_profile_read'/)
...
test('后台最小预设提供原生 Skill 目录与按需加载工具', () => {
  ...
  assert.match(serverSource, /createCharacterDesignTasks/)
  assert.match(characterDesignTasksSource, /task: 'character-design'/)
  assert.doesNotMatch(serverSource, /characterDesignDocument: snapshot\.characterDesignDocument/)
})
...
test('姿势结算通过短工具参数提交', () => {
  ...
  assert.match(flow, /request_character_design/)
  assert.doesNotMatch(flow, /CHARACTER_DESIGN_READ_TOOL|CHARACTER_DESIGN_SAVE_TOOL/)
