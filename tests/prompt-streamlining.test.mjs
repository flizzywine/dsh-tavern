...
test('卡片 Agent 以极简模式工具为底座，游玩 Agent 不暴露文件或 Skill 工具', () => {
  ...
  assert.match(orchestratorSource, /if \(mode === 'card'\) return \[shellToolName, 'str_replace_editor', 'skill', 'tavern_save_skill', 'tavern_read_card', 'tavern_read_card_raw', 'tavern_read_play_chat', 'tavern_read_worldbook', 'tavern_update_worldbook', 'tavern_read_preset', 'tavern_update_preset', 'tavern_update_card', 'tavern_restore_card'\]/)
  ...
  assert.match(serverSource, /controlledToolNames = new Set\(\['bash', 'pwsh', 'str_replace_editor', 'skill', 'tavern_save_skill', 'tavern_read_card', 'tavern_read_card_raw', 'tavern_read_play_chat', 'tavern_read_script', 'tavern_read_worldbook', 'tavern_update_worldbook', 'tavern_read_preset', 'tavern_update_preset', 'tavern_update_card', 'tavern_restore_card'\]\)/)
...
test('外部预设只作用于兼容模式，普通游玩、后台与卡片 Agent 保持 DSH 原生上下文', () => {
  const startChat = between(serverSource, 'async function startChat', 'async function appendNativeOpening')
  assert.match(startChat, /const runtimePresetSnapshot = effectiveRequestMode === 'sillytavern' \? await runtimePresets\.fullSnapshot\(\) : null/)
  ...
  assert.match(resolver, /const raw = chat\.requestMode === 'sillytavern' \? await runtimePresets\.fullSnapshot\(\) : null/)
  ...
  assert.match(serverSource, /const activePresetSnapshot = chat\.requestMode === 'sillytavern' \? await runtimePresets\.fullSnapshot\(\) : null/)
  assert.match(serverSource, /if \(!chat \|\| chat\.requestMode !== 'sillytavern'\) return \[\]/)
