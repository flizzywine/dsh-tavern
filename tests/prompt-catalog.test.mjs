...
test('固定提示词从独立 Markdown 文件完整加载', () => {
  ...
  assert.match(prompt('card-mode'), /使用完成任务所需的最简单路径/)
  ...
  assert.match(prompt('card-mode'), /世界书可以使用 `tavern_read_worldbook` 和 `tavern_update_worldbook`，也可以直接使用文件工具/)
  assert.match(prompt('card-mode'), /预设可以使用 `tavern_read_preset` 和 `tavern_update_preset`，也可以直接处理对应 JSON/)
  assert.doesNotMatch(prompt('card-mode'), /tavern_read_source/)
  assert.doesNotMatch(prompt('card-mode'), /tavern_read_script/)
  assert.match(prompt('card-mode'), /当前工作目录是 Tavern 资源根目录/)
  assert.match(prompt('card-mode'), /cards\//)
  assert.match(prompt('card-mode'), /worldbooks\//)
  assert.match(prompt('card-mode'), /presets\//)
  assert.match(prompt('card-mode'), /materials\//)
  assert.match(prompt('card-mode'), /scripts\//)
  ...
  assert.match(prompt('card-mode'), /原生文件工具、Skill、Cordis 工具或其他当前可用能力/)
  assert.match(prompt('card-mode'), /直接创建、读取、修改、重命名、组合和验证/)
  assert.match(prompt('card-mode'), /新建独立世界书.*worldbooks\/目标文件\.json/s)
  assert.match(prompt('card-mode'), /修改已经保存并生效/)
  ...
  assert.match(prompt('card-mode'), /Cordis 动态插件.*进程重启后消失/s)
  assert.match(prompt('card-mode'), /自创工具默认持久保存/)
  assert.match(prompt('card-mode'), /tools\.cordis\.yml/)
  assert.match(prompt('card-mode'), /自创 Skill 保存到系统提供的用户 Skill 目录/)
  assert.match(prompt('card-mode'), /只完成文件写入时说明已保存、需重启验证/)
