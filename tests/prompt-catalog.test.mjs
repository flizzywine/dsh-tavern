...
test('固定提示词从独立 Markdown 文件完整加载', () => {
  ...
  assert.match(prompt('card-mode'), /“人物卡库”.*“预设库”.*“世界书库”.*“剧本库”/s)
  assert.doesNotMatch(prompt('card-mode'), /破限方案库/)
  assert.match(prompt('card-mode'), /世界书通常通过 tavern_update_worldbook/)
  assert.match(prompt('card-mode'), /预设通常通过 tavern_update_preset/)
  ...
  assert.match(prompt('card-mode'), /Tavern 工具是优先路径，不是权限边界/)
  assert.match(prompt('card-mode'), /专用工具无法完成.*原生文件工具或 Cordis 创造工具/s)
  assert.match(prompt('card-mode'), /人物卡、世界书、预设、剧本和资料/)
  assert.match(prompt('card-mode'), /创建、修改、重命名或组合/)
  assert.match(prompt('card-mode'), /新建独立世界书.*worldbooks\//)
  ...
  assert.match(prompt('card-mode'), /不得把确认参数当作用户确认/)
  assert.match(prompt('card-mode'), /tavern_read_worldbook/)
  assert.match(prompt('card-mode'), /条目级最小变更/)
