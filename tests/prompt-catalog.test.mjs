...
test('固定提示词从独立 Markdown 文件完整加载', () => {
  ...
  assert.match(prompt('card-mode'), /Shell、文件读写与编辑等基础工具可以自由组合/)
  assert.match(prompt('card-mode'), /不按工具类别预设优先级/)
  assert.match(prompt('card-mode'), /明确的创建或修改请求就是操作授权/)
  assert.match(prompt('card-mode'), /不重复索取同一授权/)
  assert.match(prompt('card-mode'), /加载 `tavern-advanced-capabilities`/)
  assert.match(prompt('card-mode'), /创建、读取、修改、复制、重命名、组合和验证/)
