...
import { Session, adoptSessionEvent } from '../tavern-plugin/node_modules/@deepseek-ai/dsh-session/lib/index.js'
...
test('失败清理经过真实 DSH 消息校验及恢复，不损坏历史或误认玩家输入', () => {
  ...
  const input = { events: restored.events, nodes: restored.surface.nodes }
  ...
  assert.equal(restored.events[1].data.message.content[0].text, '原正文')
})
...
test('失败轮只有用户输入、从未获得模型来源，也能清理并恢复', () => {
  ...
  assert.doesNotThrow(() => Session.create(session.id, session.events, session.header))
})
