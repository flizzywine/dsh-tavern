...
test('后续正文不重复首轮人物卡细节，但保留姿势、Guide 和特殊指令', async () => {
  const planner = createContextPlanner({ callModel: async () => '{"ids":[]}' })
  const result = await planner.plan({
    purpose: 'body',
    card: card(),
    chat: chat([
...
test('候选项使用独立任务，不注入正文输出约束和人物卡特殊指令', async () => {
  const planner = createContextPlanner({ callModel: async () => '{"ids":[]}' })
...
test('世界书模型失败时用关键词确定性回退，不阻断正文规划', async () => {
  const planner = createContextPlanner({ callModel: async () => { throw new Error('模型不可用') }, logger: { error() {} } })
  const result = await planner.plan({
    purpose: 'body',
    card: card(),
    chat: chat(),
...
test('卡片设定与素材抽取也通过同一规划 interface', async () => {
  const planner = createContextPlanner({ callModel: async () => '{"ids":[]}' })
  const revision = await planner.plan({ purpose: 'revision', card: card(), scriptInfo: { title: '银铃', chunkCount: 3 } })
