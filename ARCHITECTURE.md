...
| Candidate Generator | `generate`、`find` | 独立生成、校验、重试和保存候选项；剧本模式在隔离上下文中用 `next`、`prev` 逐块研究或用 `search` 随机检索，再用 `point` 暂存并提交下一轮游标 |
...
4. 候选项是独立调用和独立持久化结果，不混入正文，也不复用正文输出约束。候选查阅剧本产生的推理、工具调用和返回内容也只存在于本次候选调用中。
