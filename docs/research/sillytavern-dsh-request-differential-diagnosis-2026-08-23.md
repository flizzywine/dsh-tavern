...
在固定测试条件（Infron、`google/gemini-3.7-flash`、同一人物卡、`Kemini Dramatron 陨落的天才v1.26` 预设）下，兼容模式恢复 DSH 原生流式链路后，第 4、5、6 轮连续一次请求成功，分别输出 1784、1903、1709 字，均以 `finish: stop` 结束。

随后关闭 `📽️COT（格式友好型）` 条目，第 7 轮的最终兼容请求已不再包含该条目，仍在一次请求内成功：耗时 18.137 秒、输出 1916 字、`finish: stop`。这证明当前成功不依赖该 COT 条目。

据此确认：**dsh-tavern 兼容模式已经在本固定案例中复现真实 SillyTavern 的破限能力。** 该结论是可重复的案例验收，不代表所有模型、供应商、人物卡和预设组合都必然成功。
