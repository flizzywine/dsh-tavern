...
当前人物卡支持按顺序绑定多本世界书，同一本书可以被多张卡复用。绑定关系保存在独立索引中，开局时生成本局合并快照，不迁移或改写源世界书文件。当前实现见[多世界书绑定实验](../experiments/multi-worldbook-bindings.md)；上面的 scope 模型是早期设计草案，不代表已实现通用 chat/persona 绑定。
