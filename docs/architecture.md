...
| Preset Compatibility | `inspectPreset`、`extractBypassPlan`、`activateBypassPlan`、`compileCompatibilityTurn` | 外部预设只供查看和抽取；破限方案是唯一运行资源。普通游玩投影为 DSH 三段，设置中开启的实验性兼容模式按 SillyTavern 语义编译请求 |
