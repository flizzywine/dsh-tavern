...
- 剧本候选只注入当前块，并可在独立上下文中调用只读的 `tavern_read_script`，按块号直接读取或按关键词检索整本剧本。阅读不改变游标；只有独立的 `tavern_point_script` 会提出向前定位请求。工具状态与推理不进入正文历史，生成失败不提交游标，候选 JSON 也不携带 `scriptCursor`。
...
- 当前修正：正文成功提交后游标机械前进一块；候选工具读取不推进游标，只有显式 point 可以保持或向前跳转。
- 风险：低。
- 确认：☑ 采纳　☐ 不采纳
...
2. 启动 `dsh-tavern`，逐模式 smoke：
   ...
   - script：候选项 1 个、按块号或关键词读取剧本不移动游标、显式 point 只向前定位、正文提交后前进一块
