...
正常响应事件或轮询的卡片，HTML 和依赖只初始化一次；之后由 Host Adapter 把消息、变量和 MVU 事件以增量方式送入 iframe。模板变化使用双缓冲替换；只读、只渲染一次的旧卡另有下述自动刷新兜底。
...
### 无监听旧卡的刷新兜底（2026-08-31）

《离火仙魔录》的状态栏只在初次执行和部分点击时调用 `getAllVariables()`，没有订阅变量变化。Context 增量送达不等于 DOM 会刷新；初次读到空变量时，它会一直显示“变量未就绪”。

- 仅持久状态 iframe 安装读取观察器，记录卡片实际读取的变量快照；不改原始人物卡，也不对正文、历史 iframe 做刷新兜底。
- Context 到达后给予 5 秒响应时间。已监听 `MESSAGE_UPDATED` / MVU 更新结束事件的卡片继续自行渲染；在此期间轮询重读到最新值的卡也不重建。
- 没有这些监听，且曾读取的数据与最新数据仍不一致时，请求一次双缓冲替换。新 iframe 从最新 Context 初始化，ready 前旧画面继续保留。支持首次变量晚到、后续楼层变化和变量回退；同值更新不重建。
- 宿主构造事件参数时直接读取 Context，不经过卡片读取观察器，避免把内部读取误判为卡片已刷新。
