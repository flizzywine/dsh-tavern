...
DSH 原生人物卡把一轮运行拆成两个平面：

- **叙事面**：前台主 Agent 接收玩家输入，只负责人物扮演、剧情推进和正文表达；
- **控制面**：后台能力层负责程序加载、逻辑管控、变量结算、状态推进、候选生成和 View 发布。

控制面不是另一段要求模型严格填写的文本格式，而是一条受 Host 管理的工具工作流：

```text
Host 根据人物卡声明加载能力并触发后台任务
  → 后台 Agent 理解本轮正文并作出语义判断
  → 后台 Agent 通过带 Schema 的工具提交结构化结果
  → Host 校验 branch、revision、字段和权限
  → 原子写入权威状态并生成 View 与下一轮上下文投影
```

工具调用是格式控制的 seam：后台 Agent 负责表达“要更新什么”，工具 Schema 规定“允许以什么结构更新”，Host 决定“这次更新能否生效”。状态、候选、变量和 View 数据不再编码进正文，也不再依赖模型一次性输出完整标签、JSON、HTML 注释或正则标记。

职责遵守三条规则：

1. 程序加载、任务调度、Schema 校验和原子提交由 Host 确定执行；
2. 剧情理解、状态判断和语义总结由后台 Agent 完成；
3. 后台 Agent 只能通过受限工具改变权威状态，不能直接改写前台正文。

后台任务失败时，正文仍然有效；对应状态沿用上一版或标记待结算。格式错误被限制在工具参数校验阶段，不再污染正文和前台 Session。

## 可运行能力

DSH 人物卡不是纯静态资料，也不是宿主级插件。它是一份可运行的角色资源包，可以携带受限的后台控制程序和前台 View 程序，由 dsh-tavern 统一加载、隔离和授权。
...
- 状态栏、人物面板、地图和物品栏；
- 候选项与其他玩家交互；
- 根据权威状态更新 HTML、CSS 和动画；
- 把用户操作转换成受控的 Host 事件。

运行能力分为三级：

1. **声明式能力**：状态 Schema、工作流、触发器、模板和 View 声明，默认可运行；
2. **沙箱程序**：在 QuickJS、WASM 或 iframe 等受限环境中运行，限制 CPU、内存、文件和网络访问；
3. **授权工具**：文件、网络、模型和其他高权限能力必须在 manifest 中声明，经用户确认后由 Host 执行。

人物卡程序只能通过受控 Interface 读取本卡状态、提交状态 Patch、发布候选、发布 View 数据、请求授权工具和记录诊断。所有写入必须绑定当前 branch、revision 和来源正文。

人物卡程序不能直接改写前台正文，不能读取其他人物卡或 Session 的私有数据，不能绕过 Host 操作文件系统、网络和历史状态。自定义程序失败时只停用对应能力并产生诊断，不能拖垮正文或整个游戏。

因此，DSH 人物卡虽然沿用用户熟悉的“人物卡”名称，能力上已经接近受限的小型 Agent 应用格式。
