...
酒馆人物卡的根本限制，是把叙事正文同时当作状态协议、候选协议和 UI 数据通道，再依赖正则从自由文本中恢复结构。模型一旦掉格式，状态、正文和展示便会互相污染。

DSH 原生人物卡不延续这套协议：

- 前台主 Agent 只生成叙事正文；
- 状态、候选和 View 数据由后台任务通过结构化工具提交；
- Host 校验并保存权威状态，展示器从权威状态生成 UI；
- 后台失败只影响对应状态或 View，不改写、不截取正文；
- 正则只存在于酒馆兼容 Adapter，不成为 DSH 原生人物卡的内部协议。

因此，酒馆人物卡是可导入的源格式，DSH 原生人物卡是新的编译目标。项目可以提供转换 Skill，协助用户理解原卡、提出迁移方案并在用户确认后生成原生卡；不能通过 Host 特判或自动修卡掩盖源人物卡自身的问题。
...
结构化状态、HTML View、MVU、状态更新和候选项等高级能力，不再要求前台主 Agent 一边写正文一边完成。它们统一进入**后台能力层**；酒馆正则留在兼容 Adapter，不进入原生协议：

```text
前台主 Agent：只生成正文
        ↓
后台能力工作流
    ├─ 确定性执行器：HTML 编译、安全检查、Schema 校验、Patch 应用
...
酒馆兼容 Adapter：仅对未迁移资产执行 legacy 正则、EJS、MVU 或脚本
```
...
| 常驻人物设定 | 前台稳定上下文 |
| 动态世界书匹配 | 确定性运行时 |
| 酒馆展示正则匹配与替换 | 酒馆兼容 Adapter，不进入原生协议 |
...
单项 HTML、View 或 Patch 失败只产生该能力的诊断和回退结果，不能拖垮正文或整个后台周期。兼容 Adapter 中的单条正则失败同样不得影响权威正文。
...
| 常驻世界书 | 稳定上下文 |
| 动态世界书 | 确定性召回规则 |
| 状态更新协议 | 能识别语义时转为后台结算任务、状态 Schema 与状态工具 |
| 候选项要求 | 后台候选任务 |
| 显示正则 | 能识别展示语义时转为 View 声明；否则留在兼容 Adapter |
| HTML 状态栏 | 从权威状态生成的 View renderer；原始 HTML 可作为迁移参考 |
...
转换 Skill 是迁移助手，不是静默修卡器。它应当：

1. 读取酒馆人物卡及其世界书、正则、MVU、EJS 和脚本资产；
2. 判断每项资产承担的是前台提示、后台状态、候选生成、View 展示还是兼容行为；
3. 提出 DSH 原生映射，并明确行为变化；
4. 对语义推断和不可逆转换请求用户确认；
5. 生成原生人物卡与迁移诊断，保留原始酒馆人物卡。

转换结果分为三类：

- **准确转换**：字段、生命周期和目标语义明确；
- **推测转换**：可以提出结构化方案，但需要用户确认；
- **无法转换**：保留为兼容资产或要求人工重写。

Skill 不承诺把任意文本正则机械变成工具调用。只有能识别出权威字段、触发时机和写入规则时，才生成状态 Schema 与工具；其余内容必须显式报告。
