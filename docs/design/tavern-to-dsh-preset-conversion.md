...
当前目标是把酒馆预设按有效 `prompt_order` 整理成 DSH 可理解的“前、中、后、正则”四部分。它不复刻 SillyTavern 的完整请求构造、采样器或脚本运行时，但会在新建对话时解析已支持的酒馆宏，并执行转换后的正则。

## 已确认的转换规则

- `prompt_order` 决定参与转换的条目及其相对顺序；
- 条目的初始开关以酒馆预设自身为权威：提示词读取有效 `prompt_order` 的 `enabled`，正则读取自身的 `disabled`；
...
用户只看到一个“预设”：导入后直接编辑前、中、后和正则，不需要理解源格式或转换稿。
...
新对话冻结当时的三段提示词快照；之后修改提示词不会改写旧对话，正则开关仍可实时影响投影。这里的内部运行投影不等同于 DSH Host 用 `preset.yml` 和 `agent.cordis.yml` 组成的 Agent Preset。
...
1. **前（front）**：创建会话时解析并形成稳定快照，放在系统提示词最前，后续请求继续携带同一份内容。
2. **中（middle）**：使用同一快照，每轮前台请求注入一次；后台每个候选或结算任务注入一次，不逐轮累积。
3. **后（back）**：每次前台或后台模型请求都临时追加在请求末尾；参与本次生成，请求结束后立即从有效 Session Surface 中遮蔽，不进入下一次请求上下文。

受 DSH 当前生命周期接口限制，中、后统一作为 `user` 请求消息表达；源条目的 `role` 仍保留在转换稿中，待原生请求接口可直接表达多角色尾部时再升级，不在转换阶段丢失。
...
- 所选 order 中可识别的普通提示词全部进入转换结果；
- 已关闭条目仍进入对应的 DSH 段，并保留 `enabled: false`，方便用户在 DSH 预设中重新开关；
...
转换稿保留宏原文。开始新对话时，运行时按 `front → middle → back` 以及各段内部顺序解析一次宏，三段共享同一变量状态；解析后的结果随对话快照冻结。已支持的 `setvar/getvar`、`user`、`char` 等宏在这里展开，未知宏保留原文并产生运行诊断，不由转换器猜测替换。
...
SPreset 正则不属于提示词三段，但作为 DSH 转换稿的独立“正则”部分完整映射，保留名称、开关、查找、替换、作用位置、深度、`promptOnly`、`markdownOnly`、`runOnEdit` 等字段。运行时与人物卡正则合并：输入正则作用于送模前文本，输出正则分别生成模型上下文投影和展示投影；前台与后台使用同一组启用状态。正则开关实时读取，因此无需新建对话即可重新生成已有展示投影。
