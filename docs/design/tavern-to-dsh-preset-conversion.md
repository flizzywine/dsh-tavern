...
当前目标是保留一份预设与一套条目开关，再按有效 `prompt_order` 投影成 DSH 可理解的“前、中、后、正则”四部分。兼容模式不使用 DSH 三段，而是按酒馆顺序直接使用同一批已开启条目。
...
1. **酒馆原始预设**：展示全部提示词，用户直接开启或关闭；
2. **DSH 预设预览**：展示已开启条目自动形成的前、中、后，以及完整正则。这里的开关是同一份状态的另一个入口。
...
SPreset 正则不属于提示词三段。它作为 DSH 转换稿的独立“正则”部分完整映射，保留名称、开关、查找、替换、作用位置、深度、`promptOnly`、`markdownOnly`、`runOnEdit` 等字段。正则只处理前台游玩消息的输入、Session 和展示投影，不处理后台 Agent 的结算 JSON、候选协议或历史日志。正则开关实时读取，因此无需新建对话即可重新生成已有前台展示投影。
