...
> 历史基线：本文记录既有 MVU、EJS 与 Helper 能力的实施事实；“兼容层”不再是当前顶层 module，相关能力将收敛到酒馆脚本运行模块。
...
正文重新生成采用最后一轮整体替换：保留原玩家输入，重新生成正文并重新执行后台结算；只有整轮成功才替换旧正文与模型可见投影，失败则恢复旧整轮。产品不再提供正文 Swipe 切换。

人物卡原始数据中的候选数组和变量槽位仍按兼容格式读取，但消息楼层不提供分支切换。内置世界书写入按人物卡串行；资源扫描排除 durable write 的锁与待恢复文件，损坏锁也可安全回收，避免生命周期脚本把内部锁误识别成人物卡后永久阻断写入。
