...
assert.ok(dir && process.env.DEEPSEEK_API_KEY, 'Usage: DEEPSEEK_API_KEY=... node docs/experiments/settlement-prefix-ab.mjs OUTPUT_DIR');
...
const summaries=[];
for(let repeat=1;repeat<=3;repeat++)for(const [scenario,storyText] of Object.entries(scenarios))for(const arm of (repeat%2?['tail','prefix']:['prefix','tail'])){
 const id=`${scenario}-${repeat}-${arm}`,steps=[];const caseInitial=structuredClone(initial);caseInitial.stat_data.信任=18+(repeat-1)*3;let state=structuredClone(caseInitial);
 const module=createMvuSettlementModule({model:{async run(input){
 const lines=input.system.split('\n');
 const starts=['变量通过工具提交','有变化时提交完整 operations','若 ok=false','rolledBack=true','mvu_submit_update 返回'];
 const fixed=lines.filter(line=>starts.some(start=>line.startsWith(start)));
 assert.equal(fixed.length,5);
 const remaining=lines.filter(line=>!fixed.includes(line)).join('\n');
 const conventions='【变量结算工具约定】\n执行变量结算任务时，遵守以下约定：\n'+fixed.join('\n');
 const persona='你是与前台正文生成隔离的酒馆后台 Agent。严格按本次任务协议结算。';
 const messages=[{role:'system',content:persona+(arm==='prefix'?'\n\n'+conventions:'')}, {role:'user',content:`【本轮权威状态】\n${input.turnContext}\n\n【最近剧情与本次任务】\n任务类型：状态结算\n[正文]\n${storyText}\n\n【DSH 后台任务协议（最终指令）】\n${remaining}`+(arm==='tail'?'\n\n'+conventions:'')}];
 ...
 const result=await module.settleVariables({operationId:id,chatId:id,branchId:'test',basedOnRevision:1,sessionId:id,messageId:0,swipeId:0,storyText,currentVariables:structuredClone(caseInitial),updateRules:rules,backgroundTasks:{posture:true,variables:true,characterDesign:false},system:await readFile(new URL('../../tavern-plugin/prompts/posture-settlement.md', import.meta.url),'utf8')});
